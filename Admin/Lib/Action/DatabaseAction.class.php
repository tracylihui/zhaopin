...
class DatabaseAction extends CommonAction {
    ...
    public function index() {
        ...
        $dbtables = M()->query('SHOW TABLE STATUS like "tb_%"');
		
        $total = 0;
        foreach ($dbtables as $k => $v) {
            $dbtables[$k]['size'] = $this->get_byte($v['Data_length'] + $v['Index_length']);
            $total+=$v['Data_length'] + $v['Index_length'];
        }
        $this->vlist = $dbtables;
        $this->total = $this->get_byte($total);
    ...
    public function backup() {
        ...
        $time = time();//开始时间
        $path = $this->getDbPath() . "/tables_" . date("Ymd") . "_" . $this->get_randomstr(5);
        ...
        if (strlen($sqlTable . $outstr) > 0) {
            ...
            if (!file_put_contents($file, $outstr)) {
    ...
    private function getRestoreFiles() {
        ...
        $sqlfilepre = $_GET['sqlfilepre'];//获取sql文件前缀
            ...
            while (!feof($file)) {
                ...
                if ($tem[$end] == ";") {
                    ...
                    if ($execute > 500) {
                        ...
                        $this->success('如果SQL文件卷较大(多),则可能需要几分钟甚至更久,<br/>请耐心等待完成，<font color="red">请勿刷新本页</font>，<br/>当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', U('Database/restoreData', array($this->get_randomstr(5) => $this->get_randomstr(5))));                       
                        exit();
                    }
                } else {
                    $sql.=$tem;
